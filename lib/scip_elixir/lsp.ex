defmodule ScipElixir.LSP do
  ...
  def handle_request(%Initialize{params: params}, lsp) do
    ...
    # Try to open existing index (don't block on building)
    store =
      if File.exists?(db_path) do
        try do
          ...
          e ->
            Logger.warning("[scip-elixir] Could not open index: #{inspect(e)}")
            nil
        end
      else
        nil
    ...
    {:reply,
     %InitializeResult{
       ...
       server_info: %{name: "scip-elixir", version: "0.1.5"}
